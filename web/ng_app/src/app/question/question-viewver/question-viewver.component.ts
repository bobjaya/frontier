...
export class QuestionViewverComponent implements OnInit {
  ...
  async ngOnInit() {
    await this.questionFieldService.prepareQuestions();
  ...
  getQuestionare(index) {
    if (index < 0) {
      return;
    } else if (this.questionFieldService.questionsList.length === index) {
    ...
    this._sharedService.announceProgress(index + 1);
  }
