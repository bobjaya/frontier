...
class AuthInvitation(BaseModel):
    ...
    login_link = models.ForeignKey(
        'business.LoginLink', related_name='authinvitations', null=True)
    email = models.EmailField()
    type = models.CharField(max_length=16, default=Type.RESET_PASSWORD,
                            choices=Type.CHOICES)


@receiver(post_save, sender=AuthInvitation)
def process_auth_invitation(sender, instance=None, created=False, **kw):
    from business import tasks

    if instance.state == AuthInvitation.State.PENDING:
        tasks.process_auth_invitation(instance.token)
