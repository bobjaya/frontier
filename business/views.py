...
from business.models import User, LoginLink, Administrator, Company, HiringManager, AuthInvitation
...
def reset_password(request):
    ...
    if user.login_link is None:
        login_link = LoginLink.objects.create(user=user)
        AuthInvitation.objects.create(login_link=login_link, email=email)
    else:
        AuthInvitation.objects.create(login_link=user.login_link, email=email)
